...
namespace HUNRE
{
    ...
    {
        // Chuỗi kết nối
        string connectionString = @"Data Source=JUSTIN;Initial Catalog=TRACNGHIEM;Integrated Security=True;TrustServerCertificate=True";
        ...
        // Phương thức để lấy nội dung câu hỏi và đáp án từ bảng CAUHOI dựa vào mã câu hỏi
        private (string NoiDung, string DapAn) GetCauHoiInfo(string maCH)
        {
            string noiDungCH = ""; // Khởi tạo nội dung câu hỏi
            string dapAn = ""; // Khởi tạo đáp án của câu hỏi
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                // Truy vấn để lấy nội dung câu hỏi và đáp án từ bảng CAUHOI dựa vào mã câu hỏi
                string sql = "SELECT NoiDung, DapAn FROM CAUHOI WHERE MaCH = @MaCH";
                SqlCommand cmd = new SqlCommand(sql, connection);
                cmd.Parameters.AddWithValue("@MaCH", maCH);

                // Thực thi truy vấn và lấy nội dung câu hỏi và đáp án
                SqlDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    noiDungCH = reader["NoiDung"].ToString();
                    dapAn = reader["DapAn"].ToString();
                }
                reader.Close();
            }

            return (noiDungCH, dapAn);
        }
